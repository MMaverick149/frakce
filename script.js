...
function renderMembers() {
    ...
    list.innerHTML = m_list.filter(m => m.toLowerCase().includes(q)).map(m => {
        ...
        const code = (m.length * 888 % 9000 + 1000);
        return `
            <div class="table-row">
                <div style="color:#64748b">#${code}</div>
                <div style="color:var(--accent); font-weight:800; font-size:0.75rem;">${d.rank.toUpperCase()}</div>
                <div style="font-weight:600;">${m}</div>
                <button onclick="openDossier('${m}')" class="btn-update" style="padding:5px 10px; font-size:0.65rem;">OTEVŘÍT</button>
            </div>`;
    }).join('');
}
...
function editStore(type) {
    ...
    // Zajištění, že hodnota je číslo před výpočtem
...
function openDossier(name) {
    ...
    document.getElementById('d-id-code').innerText = "#" + (name.length * 888 % 9000 + 1000);
